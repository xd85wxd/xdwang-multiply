...
图像输入模块负责接收用户上传的图片文件，并将图片保存至服务器本地目录。系统支持自然场景图片、商品图片以及文档截图等多种图像类型输入。在用户上传图片后，系统会自动完成图片存储和基础信息读取，为后续模型推理提供视觉输入。

### 会话管理模块

会话管理模块负责维护用户与系统之间的历史对话信息。系统采用 `<span>chat_history</span>` 数据结构保存用户问题与模型回答，在后续提问时将历史对话与当前问题共同输入模型，从而实现多轮连续对话功能。

此外，为避免不同图片之间的上下文信息相互干扰，系统设计了图片切换机制。当检测到用户上传新的图片时，自动清空历史会话记录并重新建立上下文环境，保证问答结果的准确性。

### 模型推理模块

模型推理模块是系统的核心组成部分，采用 Qwen2.5-VL-7B-Instruct 作为底层视觉语言模型。

该模块首先利用 Processor 对图像和文本进行预处理，然后通过 Chat Template 构建符合模型要求的输入格式，最后调用模型生成接口完成视觉理解与语言生成任务。模型能够同时处理图像信息和文本信息，实现目标识别、场景理解、文字识别以及基础视觉推理等功能。

### 前端展示模块

前端展示模块采用 HTML、CSS 和 JavaScript 实现，为用户提供图形化交互界面。用户可以通过浏览器上传图片、输入问题并查看模型回答。
